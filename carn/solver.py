...
class Solver():
        ...
        while True:
            for inputs in self.train_loader:
                ...
                if self.step % cfg.print_interval == 0:
                    if cfg.scale > 0:
                        psnr = self.evaluate("dataset/Set", scale=cfg.scale, num_step=self.step)
                        self.writer.add_scalar("Set", psnr, self.step)
                    else:    
                        psnr = [self.evaluate("dataset/Set5", scale=i, num_step=self.step) for i in range(2, 5)]
                        self.writer.add_scalar("Set5_2x", psnr[0], self.step)
                        self.writer.add_scalar("Set5_3x", psnr[1], self.step)
                        self.writer.add_scalar("Set5_4x", psnr[2], self.step)
    ...
    def decay_learning_rate(self):
        lr = self.cfg.lr * (0.85 ** (self.step // self.cfg.decay))
        return lr
